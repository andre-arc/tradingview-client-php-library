...
class AuthorizationApi
{
     ...
     * @param  \OpenAPI\Client\Model\Locale $locale locale (required)
     *
     * @throws \OpenAPI\Client\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     * @return OneOfAuthorizeResponseErrorResponse
     */
    public function authorize($login, $password, $locale)
    {
        list($response) = $this->authorizeWithHttpInfo($login, $password, $locale);
        return $response;
    }
     ...
     * @param  string $login User login. (required)
     * @param  string $password User password. (required)
     * @param  \OpenAPI\Client\Model\Locale $locale (required)
     *
     * @throws \OpenAPI\Client\ApiException on non-2xx response
     * @throws \InvalidArgumentException
     * @return array of OneOfAuthorizeResponseErrorResponse, HTTP status code, HTTP response headers (array of strings)
     */
    public function authorizeWithHttpInfo($login, $password, $locale)
    {
        $request = $this->authorizeRequest($login, $password, $locale);
        ...
        try {
            ...
            switch($statusCode) {
                case 200:
                    if ('OneOfAuthorizeResponseErrorResponse' === '\SplFileObject') {
                    ...
                    return [
                        ObjectSerializer::deserialize($content, 'OneOfAuthorizeResponseErrorResponse', []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
            }

            $returnType = 'OneOfAuthorizeResponseErrorResponse';
            ...
            switch ($e->getCode()) {
                case 200:
                    $data = ObjectSerializer::deserialize(
                        $e->getResponseBody(),
                        'OneOfAuthorizeResponseErrorResponse',
     ...
     */
    public function authorizeAsync($login, $password, $locale)
    {
        return $this->authorizeAsyncWithHttpInfo($login, $password, $locale)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function authorizeAsyncWithHttpInfo($login, $password, $locale)
    {
        $returnType = 'OneOfAuthorizeResponseErrorResponse';
        $request = $this->authorizeRequest($login, $password, $locale);
     ...
     */
    public function authorizeRequest($login, $password, $locale)
    {
        // verify the required parameter 'login' is set
        if ($login === null || (is_array($login) && count($login) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $login when calling authorize'
            );
        }
        ...
        if ($locale === null || (is_array($locale) && count($locale) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $locale when calling authorize'
            );
        }
