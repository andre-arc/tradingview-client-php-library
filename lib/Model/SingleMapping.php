...
/**
 * SingleMapping Class Doc Comment
 *
 * @category Class
 ...
 */
class SingleMapping implements ModelInterface, ArrayAccess, \JsonSerializable
{
    ...
    /**
      ...
      * @var string
      */
    protected static $openAPIModelName = 'SingleMapping';
    ...
    public function listInvalidProperties()
    {
        ...
        if ((count($this->container['f']) < 1)) {
            $invalidProperties[] = "invalid value for 'f', number of items must be greater than or equal to 1.";
        }
     ...
     * Sets f
     *
     * @param string[] $f An array with the only one string element &ndash; broker symbol name.
     *
     * @return self
     */
    public function setF($f)
    {
        ...
        if ((count($f) < 1)) {
            throw new \InvalidArgumentException('invalid length for $f when calling SingleMapping., number of items must be greater than or equal to 1.');
        }
        $this->container['f'] = $f;
     ...
     * Sets s
     *
     * @param string $s TradingView symbol name with a prefix (AA:XXXX).
     *
     * @return self
     */
